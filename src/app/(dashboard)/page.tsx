    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 pb-10">
      ...
      <div className="lg:col-span-8 space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          {/* Overview Widget */}
          <Card className="md:col-span-2 bg-card border border-border/40 rounded-[2.5rem] p-8 card-shadow-md overflow-hidden relative group fluid-hover">
            <div className="flex items-center justify-between mb-6">
              <div className="space-y-1">
                <h3 className="text-lg font-bold text-foreground tracking-tight">Financial Trends</h3>
                <p className="text-[10px] uppercase font-black tracking-[0.2em] text-muted-foreground opacity-50">Last 7 Days Overview</p>
              </div>
              <div className="flex flex-col items-end">
                <span className="text-xs font-bold text-muted-foreground uppercase tracking-widest bg-muted px-3 py-1 rounded-full border border-border">
            ...
            <div className="grid grid-cols-3 gap-4 mb-8">
              <div className="flex flex-col">
                <span className="text-2xl font-black text-foreground">{transactionCount}</span>
                <span className="text-[9px] uppercase font-black tracking-widest text-muted-foreground mt-1 opacity-60">Total TX</span>
              </div>
              <div className="flex flex-col">
                <span className="text-2xl font-black text-emerald-500">{transactions.filter(t => t.type === 'Income').length}</span>
                <span className="text-[9px] uppercase font-black tracking-widest text-emerald-500 mt-1 opacity-60">Incomes</span>
              </div>
              <div className="flex flex-col">
          ...
          {/* Your Balance Card */}
          <Card className="bg-card border border-border/40 rounded-[2.5rem] p-8 card-shadow-md relative overflow-hidden flex flex-col fluid-hover">
             <div className="flex items-center justify-between mb-8">
               <h3 className="text-lg font-bold text-foreground tracking-tight">Your Balance</h3>
               <Badge variant="outline" className="bg-muted border-border text-muted-foreground text-[10px] uppercase font-bold tracking-widest">{currency} <ArrowUpRight className="ml-1 h-3 w-3" /></Badge>
             </div>

             <div className="space-y-1 mb-8">
               <p className="text-xs font-bold text-muted-foreground uppercase tracking-widest">Balance</p>
               <div className="flex items-end gap-3">
                 <h2 className="text-4xl font-black text-foreground tracking-tight">{formatCurrency(balance).replace('$', '').trim()}</h2>
             ...
             <div className="mt-auto relative z-10">
                <div className="bg-gradient-to-br from-primary/5 to-transparent p-6 rounded-[2rem] border border-border/50 relative overflow-hidden group">
                  <div className="flex items-center justify-center gap-3 mb-4">
                    <Sparkles className="h-4 w-4 text-primary animate-pulse" />
                    <span className="text-xs font-bold text-foreground tracking-wide">Finance Health</span>
                  </div>
        ...
        <Card className="bg-card border border-border/40 rounded-[2.5rem] p-8 card-shadow-md overflow-hidden">
          <div className="flex items-center justify-between mb-8">
            <h3 className="text-lg font-bold text-foreground tracking-tight">Recent Transactions</h3>
            <Button variant="outline" size="sm" className="bg-muted border-border rounded-full text-xs font-bold text-muted-foreground gap-2 hover:text-foreground">
          ...
          <div className="space-y-0 text-sm">
            ...
            {recentTransactions.map((t, i) => {
              ...
              return (
                <div key={t.id} className="grid grid-cols-12 gap-4 py-6 border-b border-border group hover:bg-muted/50 transition-colors rounded-3xl px-1">
                  <div className="col-span-7 flex items-center gap-4">
                    <div className={cn(
                      "h-12 w-12 rounded-2xl flex items-center justify-center shrink-0 border border-border",
                  ...
                  <div className="col-span-5 flex flex-col justify-center text-right">
                    <span className={cn("font-black text-xl tracking-tighter", t.type === 'Income' ? "text-emerald-500" : "text-foreground")}>
      ...
      <div className="lg:col-span-4 space-y-8">
        ...
        <Card className="bg-card border border-border/40 rounded-[2.5rem] p-8 card-shadow-md relative overflow-hidden group fluid-hover">
          <div className="absolute top-0 right-0 p-6 opacity-5 group-hover:opacity-10 transition-opacity">
            <TrendingUp className="h-24 w-24 text-emerald-500" />
          </div>
          
          <div className="flex items-center justify-between mb-8 relative z-10">
            <h3 className="text-lg font-bold text-foreground tracking-tight">Income Summary</h3>
          ...
          <div className="space-y-6 relative z-10">
            ...
            <div className="space-y-4 pt-4 border-t border-border">
              <div className="flex items-center justify-between">
                ...
                <span className="text-xs font-black text-emerald-500 bg-emerald-500/10 px-2 py-0.5 rounded-md">+12.5%</span>
              </div>
            ...
            <Button 
              className="w-full mt-4 h-12 bg-muted hover:bg-muted/80 text-foreground font-bold text-[10px] uppercase tracking-[0.2em] rounded-2xl border border-border transition-all"
              onClick={() => {
        ...
        <Card className="bg-primary/5 border border-primary/20 rounded-[2.5rem] p-8 shadow-lg relative overflow-hidden group fluid-hover">
           <div className="flex flex-col items-center text-center space-y-4">
              <div className="h-12 w-12 bg-primary/20 rounded-2xl flex items-center justify-center text-primary">
                <Target className="h-6 w-6" />
              </div>
              <div className="space-y-1">
                <h4 className="text-sm font-bold text-foreground">Savings Progress</h4>
